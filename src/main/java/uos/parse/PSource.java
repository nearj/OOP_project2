...
/**
 * Parsing with requested source file(Frankly, just text contents) class. Implements
...
public final class PSource implements Parse {
	...
	private PSource( PSource pSource ) {
		this.classList = pSource.getClassList();
		this.fileName = pSource.getFileName();
	};
	...
	public static PSource newInstance( PSource pSource ) {
		return new PSource( pSource );
	}
	...
	public String getContents() {
		...
		for ( Classes classes : classList ) {
			...
			strBuilder.append( pClass.getField() + Delim.INITIAL_OPEN + Delim.LINE_FEED + pClass.getContents()
					+ Delim.LINE_FEED + Delim.INITIAL_CLOSE + Delim.LINE_FEED + Delim.LINE_FEED );
			for ( Methods methods : classes.getMethodList() ) {
				PMethod pMethod = (PMethod) methods;
				strBuilder.append( pMethod.getField() + Delim.CLAUSE_OPEN + Delim.LINE_FEED + pMethod.getContents()
	...
	public void replaceClass( Classes classes_Src, Classes classes_Dst ) {
		classList.set( classList.indexOf( classes_Dst ), classes_Src );
	}
	 ...
	 * <p>
	 * <p>
	 * Accept the complete source level contents which has multiple class in the source.<br>
	 * If format of contents is not complete, there could be unpredicted error.
	 */
	@Override
	public void setContents( String contents ) {
		...
		while ( scn.hasNext() ) {
			...
			String className = totalContent.substring( 0, totalContent.indexOf( Delim.INITIAL_OPEN ) - 1 );
			...
			String methodContents = totalContent.substring( totalContent.indexOf( Delim.INITIAL_CLOSE ) + 2 ).trim();
	...
	private void initParser( String initContent ) {
		...
		while ( initScn.hasNext() ) {
			...
			String contents = initScn.next().replace( Delim.LINE_FEED, Delim.BLANK ).replace( Delim.LINE_END,
					Delim.LINE_END + Delim.SPACE );
			...
			while ( lineScn.hasNext() ) {
				...
				if ( !lineScn.hasNext() ) {
					if ( !lineContents.trim().equals( Delim.BLANK ) )
						acModifier = AccessModifier.set( lineContents.trim() );
					break;
				}
				...
				if ( isMember ) { // If line content is member.
					...
					Type retrunType = Type
							.set( lineContents.substring( 0, lineContents.indexOf( Delim.SPACE ) ).trim() );
					String methodName;
					...
					pMember.setAccessModifier( acModifier );
					...
					String methodInfo = lineContents.substring( 0, lineContents.indexOf( Delim.PARAMETER_OPEN ) )
							.trim();
					...
					case Deconstructor:
						...
						break;
					case Default: {
						pMethod.setName( methodInfo.substring( methodInfo.indexOf( " " ) ).trim() );
						pMethod.setReturnType(
								Type.set( methodInfo.substring( 0, methodInfo.indexOf( " " ) ).trim() ) );
					}
						break;
					}
	...
	private void methodParser( String methodContents ) {
		...
		Scanner methodScn = new Scanner( methodContents.replace( Delim.CLAUSE_OPEN + Delim.CLUASE_CLOSE,
		...
		/*
		 * regex: {?}, java syntax: (?<=\\{)\\} '\\{' : Firstly, use clause open capturing group
	...
	private void methodParseHelper( String contents ) {
		...
		String infoContent_Prev = infoContent.substring( 0, infoContent.indexOf( classes.getName() + Delim.ACCESSOR ) )
				.trim();
		String infoContent_Post = infoContent.substring( infoContent.indexOf( classes.getName() + Delim.ACCESSOR )
				+ ( classes.getName() + Delim.ACCESSOR ).length() );
		
		// < Setting name and parameter >
		String methodName = infoContent_Post.substring( 0, infoContent_Post.indexOf( Delim.PARAMETER_OPEN ) ).trim();
		...
		if ( paramContent.contains( Delim.COMMA ) ) {
			...
			while ( paramScn.hasNext() ) {
				...
				Type paramType = Type.set( paramElem.substring( paramElem.indexOf( Delim.SPACE ) + 1 ).trim() );
	...
	//
	// Although this method get desired result usally, because this method check lines by
	// '!'(U+0031)
